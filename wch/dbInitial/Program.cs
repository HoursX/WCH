...
namespace dbInitial
{
    class Program
    {
        ...
        {
            string sql = @"../../../../sql/wch.sql";
            string machine = @"(localdb)\MSSQLLocalDB";
            string other = "510-58";
            string path = Path.GetFullPath(sql);
            Console.WriteLine("文件路径：" + path);
            Console.WriteLine("数据库名：" + machine);
            ProcessStartInfo info = new ProcessStartInfo("sqlcmd", @" -S " + other + " -U sa -P 123456 " + " -i " + path);
            //禁用OS Shell
            ...
            //启动进程
            proc.Start();
            Console.WriteLine("数据库写入成功，按任意键退出");
            Console.ReadKey();
        }
        ...
        public static void Insertclass()
        {
            ...
            string s = "ABCDEF";
            int d = 100;
            string path = @"../../../../sql/data.sql";
            StreamWriter file = new StreamWriter(Path.GetFullPath(path), true);
            while(d-- > 0)
            {
                for (int i = 1; i <= 15; i++)
                {
                    classname = "1" + random.Next(16, 20).ToString() + random.Next(0, 100).ToString("000") + s[random.Next(0, 6)];
                    string sql = string.Format("INSERT INTO wch_Class (ClassName, DepID) VALUES ('{0}', '{1}')", classname, i);
                ...
                for (int b = 30; b <= 35; ++b)
                {
                    for (int c = 1; c <= 20; ++c)
                    {
                        ...
                        if (random.Next(0, 2) == 1)
                            name = fname[random.Next(0, fname.Count())] + lname[random.Next(0, lname.Count())];
                        else
                            name = fname[random.Next(0, fname.Count())] + lname[random.Next(0, lname.Count())] + lname[random.Next(0, lname.Count())];
                        string id = "000" + a + b.ToString("000") + c.ToString("000");

                        bool sex = random.Next(0, 2) == 1;

                        int Class = random.Next(1, 71);

                        int DepID = random.Next(1, 15);

                        int StuAge = random.Next(25, 65);
                        string Grade = "20" + a + "-09-01";

                        string sql = string.Format("INSERT INTO wch_Teacher (TeaID, TeaName, Gender, DepID, TeaAge) VALUES ('{0}', '{1}','{2}','{3}','{4}')", id, name, sex, DepID, StuAge);
                        Console.WriteLine(sql);
                        file.WriteLine(sql);
                    }
                }
